...
<div class="container mx-auto px-6 py-8">
    <h2 class="text-3xl font-bold text-gray-800 mb-6">Edit Stok</h2>

    <div class="bg-white shadow-md rounded-lg p-6 mx-auto">
        <form id="editStokForm" method="POST" action="{{ route('ppic.qcGagal.update', $gudangQcGagal->gudang_qc_gagal_id) }}">
            @csrf
            @method('PUT')
                ...
                <select id="barang_id" name="barang_id" required
                    ...
                    @foreach ($barang as $b)
                        <option value="{{ $b->barang_id }}" 
                            {{ $gudangQcGagal->barang_id == $b->barang_id ? 'selected' : '' }}>
                            {{ $b->nama_barang }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select id="sub_proses_id" name="sub_proses_id" required
                    ...
                    @foreach ($subProses as $sp)
                        <option value="{{ $sp->sub_proses_id }}" 
                            {{ $gudangQcGagal->sub_proses_id == $sp->sub_proses_id ? 'selected' : '' }}>
                            {{ $sp->nama_sub_proses }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <input type="number" id="jumlah" name="jumlah" min="1" required
                    value="{{ old('jumlah', $gudangQcGagal->jumlah) }}"
            ...
            <div class="mb-4">
                ...
                <input type="text" id="asal_spk" name="asal_spk"
                    value="{{ old('asal_spk', $gudangQcGagal->asal_spk) }}"
                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2">
                @error('asal_spk')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-6">
                ...
                <textarea id="catatan" name="catatan" rows="3"
                    class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 p-2">{{ old('catatan', $gudangQcGagal->catatan) }}</textarea>
                @error('catatan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
