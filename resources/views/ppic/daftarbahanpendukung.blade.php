...
<div class="w-full mx-auto mt-4 mb-5 px-6">
    <h2 class="text-3xl font-bold text-center text-gray-800 mb-6">Daftar Stok Bahan Pendukung</h2>
                ...
                @forelse($bahanPendukung as $index => $bahan)
                    <div class="bg-white rounded-2xl shadow-md p-5 border relative">
                        ...
                        <div class="flex justify-end gap-4 mt-4 pt-3 border-t">
                            <a href="{{ route('ppic.daftarbahanpendukung.edit', $bahan->bahan_pendukung_id) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-blue-500 text-white text-sm font-semibold rounded-lg shadow hover:bg-blue-700 transition-all duration-200">✏️ Edit</a>
                            <form action="{{ route('ppic.daftarbahanpendukung.destroy', $bahan->bahan_pendukung_id) }}" method="POST" data-swal-delete="{{ $bahan->nama_bahan_pendukung }}">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="inline-flex items-center gap-2 px-4 py-2 bg-red-500 text-white text-sm font-semibold rounded-lg shadow hover:bg-red-700 transition-all duration-200">🗑️ Hapus
                                </button>
                            </form>
                        </div>
                    </div>
                            ...
                            <tr class="hover:bg-gray-100 transition">
                                ...
                                <td class="py-3 px-4">
                                    <div class="flex flex-col gap-2 items-start">
                                        ...
                                        <a href="{{ route('ppic.daftarbahanpendukung.edit', $bahan->bahan_pendukung_id) }}"
                                        ...
                                        <form action="{{ route('ppic.daftarbahanpendukung.destroy', $bahan->bahan_pendukung_id) }}" 
                                            method="POST" 
