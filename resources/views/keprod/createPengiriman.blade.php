...
@section('content')
    <div class="max-w-4xl mx-auto bg-white p-8 rounded-xl shadow-md mt-8">
        ...
        <h2 class="text-2xl font-bold mb-6 text-gray-700">Tambah Pengiriman</h2>
        ...
        <form action="{{ route('keprod.pengiriman.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <select name="qc_id" required class="w-full mt-1 px-3 py-2 border rounded-lg">
                    ...
                    @foreach($qc as $user)
                        <option value="{{ $user->id }}" {{ old('qc_id') == $user->id ? 'selected' : '' }}>{{ $user->name }}
                        </option>
                    @endforeach
                </select>
                @error('qc_id')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
            </div>
            ...
            <div>
                ...
                <select name="supir_id" required class="w-full mt-1 px-3 py-2 border rounded-lg">
                    ...
                    @foreach($supir as $user)
                        <option value="{{ $user->id }}" {{ old('supir_id') == $user->id ? 'selected' : '' }}>{{ $user->name }}
                        </option>
                    @endforeach
                </select>
                @error('supir_id')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
            </div>
            ...
            <div>
                ...
                <select name="kendaraan_id" required class="w-full mt-1 px-3 py-2 border rounded-lg">
                    ...
                    @foreach($kendaraan as $k)
                        <option value="{{ $k->kendaraan_id }}" {{ old('kendaraan_id') == $k->kendaraan_id ? 'selected' : '' }}>
                            {{ $k->nama }}</option>
                    @endforeach
                </select>
                @error('kendaraan_id')<div class="text-red-600 text-sm mt-1">{{ $message }}</div>@enderror
            </div>
            ...
            <script>
                function initEvents() {
                    ...
                    document.querySelectorAll('.useBahanCheckbox').forEach(chk => {
                        ...
                        if (chk.checked) {
                            ...
                            // delay a bit to allow sub-proses load to finish
                            setTimeout(() => loadBahanPendukung(item), 250);
                        }
                    });

                    // Produksi -> load sub proses saat ini
                    document.querySelectorAll('.produksiSelect').forEach(sel => {
                        ...
                        // Trigger ketika value berubah
                        sel.addEventListener('change', function () { loadSub.call(this); });

                        // Trigger meski user memilih pilihan yang sama
                        sel.addEventListener('click', function () { loadSub.call(this); });

                        // Initialize if there's an old sub-proses selected
                ...
                async function loadBahanPendukung(item) {
                    ...
                    const subProsesTujuan = item.querySelector('.subProsesTujuanSelect').value; // ambil dari form
                ...
                Sub proses tujuan tidak boleh kurang atau sama dengan sub proses saat ini!
                    ...
                    const res = await fetch(`/keprod/bahan-pendukung/${produksiId}/${subProsesTujuan}?current=${subProsesSaatIni}`);
                    const data = await res.json();
